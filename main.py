...
class Chek : 
    ...
    def __init__(self , image_model:str , promth_text:str , num_arms:int):
        ...
        self.promth_text = promth_text
        self.promth_predict = Autrain.Predicts('GTNet.pth' ,  self.promth_text)
        al_dir = r'C:\Users\user\Documents\Thinklab\WebServer\TheGalery\DataGambar' + f'\{self.promth_predict[0]}'
        self.image_sl = MAB_Env(num_arms , al_dir)
        self.image_slected = self.image_sl.image_Slection()
        
    def Img_to_Vec(self , imdir): 
        img = tf.keras.preprocessing.image.load_img(imdir , target_size=(100 , 100))
        vec = tf.keras.preprocessing.image.img_to_array(img) / 255 
...
promth = st.chat_input('ada yang bisa saya bantu ?')
if promth : 
